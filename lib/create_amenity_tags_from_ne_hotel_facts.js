...
function create_amenity_tag_from_hotel_facts (ne_hotel) {
	ne_hotel.facts.forEach(function(fact) {
		...
		if(fact.id.match(/internet/i)) { // Someone decided 'a la carte restaurant' was a good id ...
			// console.log(fact.id);
			fact.id = 'wifi';
		}
		var amenity_id = 'amenity:' + fact.id.toString().toLowerCase(); // AllInclusive >> allinclusive
		tag['_id'] = amenity_id
...
Object.keys(all_ne_hotels).forEach(function(id) {
	var tag = create_amenity_tag_from_hotel_facts(all_ne_hotels[id]);
})
