...
// all 42k master hotel records with their 
...
function format_ne_hotel_as_taggable_tag (ne_hotel) {
  ne_hotel_with_master_id = ne_hotels_map[ne_hotel.wvId];
  var record = {
		...
		tags: [
			{
				tagId: 'hotel:mhid.' + ne_hotel.MHID || 'NO_MHID', // only 2/3 of NE Hotels are Mapped to MHID
				source: 'master_hotel_mapping',
				inherited: false,
				active: ne_hotel.MHID ? true : false
			}
		],
		metadata: []
	};

	record.metadata[record.metadata.length + 1] = ne_hotel.TripadvisorLocationID ? {
		key: 'tripadvisor',
